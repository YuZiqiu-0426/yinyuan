...
| 命令执行 (cmd:execute) | `EventType::Command`；协议含 `command_plugin` 能力位，**当前 relay 未按该位做路由强制**（见 `docs/product/任务清单-v2.md` SV-04 等） | 协议匹配 / 强制待接入 |
...
- timestamp -> 包体顶层 `timestamp`（或放入 `metadata` 作补充，见 `docs/orchestration/agent-collab-protocol-v1.md`）
