...
**产品边界（与 §5 一致）**：普通用户侧 **Web 聊天客户端（y2-chat）已移除**，用户交互为 **CLI-only**（`y2m`）；**管理端 Web**（Angular）仍为 v1 交付范围。第一阶段以 **统一认证中心 + RBAC 权限模型** 落地为主；**消息总线已具备的能力**（如 CLI 侧 `text` / `json` / `command` / `file`）以 [`当前实现说明.md`](./当前实现说明.md) 为事实基线，本规格描述 **接入认证后的目标约束**，避免与 relay 当前未鉴权实现相混淆。
...
> **目标行为**（`auth-service` 接入 **`y2m-server`** 的 `init` / 路由链路之后生效）。**当前事实**：`y2m-server` 的 `handle_init` 未校验 `InitPayload.token`、未调用 `POST /auth/introspect`，事件路由未按原子权限或会话状态拦截 — 以 [`当前实现说明.md`](./当前实现说明.md) 为准。
