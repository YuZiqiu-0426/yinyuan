...
├── docs/                   ← 设计、规范、进度、实现说明
├── CONVENTIONS.md          ← 工程约定（代码规模、Git、tmux、pnpm）
├── CLAUDE.md               ← 仓库导航、构建/测试入口、架构速览
└── README.md               ← 本文件（项目总入口）
```
...
| [`CONVENTIONS.md`](CONVENTIONS.md) | 工程约定：函数/文件行数上限、Git 提交、tmux、pnpm | 🟡 高 |
| [`CLAUDE.md`](CLAUDE.md) | 仓库导航、构建命令、测试分类、架构说明 | 🟡 高 |

> **冲突规则**：若文档之间描述不一致，以 `当前实现说明.md` > `工作进度.md` > `CONVENTIONS.md` > `CLAUDE.md` > 其他设计/API 文档为准。
